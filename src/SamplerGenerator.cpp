...
template<typename EdgeData>
void SamplerGenerator<EdgeData>::set_predicate_fun(std::function<bool(SubGraph<EdgeData>)> predicate_fun_) {
	used_predicate_fun = true;
	predicate_fun = predicate_fun_;
}
...
double SamplerGenerator<EdgeData>::estimate() {
	...
	if (used_predicate_fun) {
		if (! predicate_fun(sampled_subgraph)) {
			return 0;
		}
	}
	for (VertexId p_i = 0; p_i < num_unique_patterns; ++ p_i) { // could be changed to unique patterns TODO
		if (unique_patterns[p_i]->is_automorphic(num_vertices, num_edges, edges)) {
