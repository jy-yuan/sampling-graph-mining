...
int main(int argc, char ** argv) {
	ofstream fout_ascii("../dataset/simple-10.edgelist");
	ofstream fout_binary("../dataset/simple-10.biedgelist");

	srand(time(NULL));
	set<pair<VertexId, VertexId>> used_edges;
	const VertexId num_vertices = 10;
	const EdgeId num_edges = num_vertices * 4;
