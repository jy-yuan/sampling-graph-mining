...
int main(int argc, char ** argv) {
	...
	EdgeId five_chain_count = 0;
	for (VertexId v_0 = 0; v_0 < vertices; ++ v_0) { // the first vertex
		if (v_0 % (vertices / 10) == 0 || v_0 == vertices - 1) {
			...
			for (Edge * p_1 = edge_list[v_1]; p_1; p_1 = p_1->next) {
				...
				if (v_2 != v_0) {
					for (Edge * p_2 = edge_list[v_2]; p_2; p_2 = p_2->next) {
						...
						if (v_3 != v_0 && v_3 != v_1) {
							for (Edge * p_3 = edge_list[v_3]; p_3; p_3 = p_3->next) {
								VertexId v_4 = p_3->dst; // the fifth vertex
								if (v_4 != v_0 && v_4 != v_1 && v_4 != v_2) {
									++ five_chain_count;
								}
							}
						}
	...
	cout << endl;
	cout << "count: " << five_chain_count / 2 << endl;
	return 0;
}
